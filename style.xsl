...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0">
  ...
  <xsl:template match="tei:titleStmt|tei:publicationStmt|tei:msIdentifier"></xsl:template>
  ...
  <xsl:template match="tei:supplied[@reason = 'omitted' ]"><xsl:text disable-output-escaping="yes"><![CDATA[&lsaquo;]]></xsl:text><xsl:value-of select="."/><xsl:text disable-output-escaping="yes"><![CDATA[&rsaquo;]]></xsl:text></xsl:template>

  <xsl:template match="tei:div[@type = 'textpart' ]">
    <div class="content textpart">
      <xsl:apply-templates/>
    </div>
  </xsl:template>
